...
void update(ll segtree[], ll lazy[], ll ldx, ll hdx, ll val, ll low, ll high, ll pos)
{
    ...
    ll mid=(low+high)>>1;
    update(segtree,lazy,ldx,hdx,val,low,mid,2*pos);
    update(segtree,lazy,ldx,hdx,val,mid+1,high,2*pos+1);
    segtree[pos]+=val*(min(hdx,high)-max(ldx,low)+1);
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        while(c--)
        {
            ...
            {
                cin>>p>>q>>v;
                update(segtree,lazy,p,q,v,1,n,1);
            }
            else
            {
                cin>>p>>q;
